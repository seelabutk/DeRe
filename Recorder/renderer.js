...
function execute(command, callback) {
    ...
    console.log('execing: ', commands.join(" "));
    let process = execFile(commands.splice(0, 1)[0], commands);
    process.on("close", function(data){
        console.log('done: ' + data);
        if(callback)    callback(data, window.data);
    });
}
...
function spawn(command, callback) {
    ...
    let commands = command.split(" ");
    console.log('spawning: ' + commands.join(' '));
    ...
    process.on("close", function(code){
        ...
        if(callback)    callback(code, window.data);
    });
}
...
{
    c.rad = Math.sqrt(Math.pow(c.midX - c.endX, 2) + Math.pow(c.midY - c.endY, 2));
    ctx.beginPath();
...
{
    document.querySelectorAll(`.tab-item[data-tab-id="${tab}"]`)[0].remove();
    //todo: remove all targets associated with tab and redraw?
    draw(null, null, null, true);
}
...
function addTab()
{
    ...
    tab_close_icon.setAttribute("close", tab_id);
...
document.querySelector('.tab-group').addEventListener("click", function(e){ 
    if (e.target.getAttribute("data-tab-action") == "add") {
        return addTab();
    } else if(e.target.getAttribute("close")) {
        removeTab(e.target.getAttribute("close"));
    }
